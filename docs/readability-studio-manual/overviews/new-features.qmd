...
## {{< var PROGRAM_NAME >}} 2026.1 {-}
***

### General Enhancements {-}

- Added a **Preview** page to the project wizard.
  This shows an excerpt of your document to preview how it will appear with your current settings.
- Added the [**Plain Language Guide**](#sec-reviewing-plain-language-guide) to standard projects.
  This report flags technical phrases that aren't explained nearby in your document.
  - A **Programming** phrase list is currently available.
- Redesigned the **Export All** single-file HTML report for standard projects.
  - Added a branded masthead banner.
  - Added a sticky, numbered table of contents sidebar.
  - Each section can now be collapsed or expanded from its heading.
- Export options are now located under a dedicated `r keys("Export")` button (on the ribbon), separate from the `r keys("Save")` button.
- Added color balance, despeckle, and sharpen effects for plot background images.
- Added **Merge Images** and **Apply Effect** tools to the **Image** section on the **Tools** tab of the ribbon.
  - Includes new features for postprocessing documents scanned as images.
- Highlighted-text windows (e.g., **3+ syllable**) have been rebuilt to render as adaptive HTML.
  - Now supports the program's light/dark theme, as well as the selected program theme.
  - Added tooltips to highlighted words and phrases, explaining why each one was highlighted.
  - Added "back-to-top" elevator buttons.
  - Added an **Underline** option for highlighting text, in addition to the existing background and font color options.

### Analysis/Test Enhancements {-}

- Improved document indexing: [Aggressively exclude](#sec-options-text-exclusion) will now also exclude sentences that are really just table or figure captions (e.g., *Table 1.*, *Figure 5A.*).

### Importing/Exporting Enhancements {-}

- Batch projects can now analyze cells inside of *OpenDocument* Spreadsheet (\*.ods) files, the same way as *Excel* (\*.xlsx) files.
...
- Modernized the look of summary reports.
- Reports can now be exported as PDF.
- Added a **Theme** option, allowing you to select the accent colors used for banners, notes, and warnings.

### List Enhancements {-}

- Lists can now be saved as PDF.
- Lists exported as HTML now support light and dark mode in your browser.
