...
To export any window, go to the **Home** tab and click the `r keys("Export")` button, or right-click on the window and select **Save**.

Numerous formats are available for each type of window:

- [Highlighted text](#sec-text-window-export): HTML, RTF, and PDF
...
All windows can also be exported at once by clicking the arrow beneath `r keys("Export")` and selecting [**Export All...**](#sec-export-all-options).
...
Select **Export All**\index{exporting!projects} from the `r keys("Export")` button's menu to display the **Export All Options** dialog.
...
**Export summary reports as**: Select the file type that you want to save summary reports (e.g., test scores, statistics) as from this list.
This option only applies to standard projects, and when exporting as separate files.
...
To do this, click on the **Home** tab of the ribbon, click on the arrow below the `r keys("Export")` button, and select **Export Filtered Document**.

`r menu(c('Home','Export','Export Filtered Document'))`
