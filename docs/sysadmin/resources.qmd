...
- "base-personal.txt": person names should go in here
- `r file_path(c("abbreviations"))`:
  - "abbreviations.txt": list of abbreviations that the program recognizes in text.
    (Words in this file must include their periods.)
  - "non-abbreviations.txt": list of words that should never be seen as abbreviations.
...
The next time that the *CMake* build process is run, all compiled \*.mo files will be copied into the build and installer folders.
